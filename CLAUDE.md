...
- **Matrix shorthands:** Three shorthand commands expand inside any math span before KaTeX renders — no extra packages needed. Commas separate columns, semicolons separate rows: `\mat{a,b;c,d}` → `\begin{pmatrix}` (parentheses), `\bmat{a,b;c,d}` → `\begin{bmatrix}` (square brackets), `\vmat{a,b;c,d}` → `\begin{vmatrix}` (determinant). Implemented in `_expand_matrix_shorthands()` in `note_editor.py`, called during `_restore_math()` before placeholders are written back to HTML.
